...
const limiter = rateLimit({
  ...
  max: parseInt(process.env.RATE_LIMIT_MAX_REQUESTS) || 700, // 700 requests per 15 minutes (production-friendly)
  message: {
    success: false,
  ...
  legacyHeaders: false,
  skip: (req) => {
    // Skip rate limiting for health checks and critical user actions
    return req.path === '/health' || 
           req.path === '/db-health' ||
           req.path === '/api/auth/switch-role'; // User-initiated role switching shouldn't be rate limited
  }
});
